...
class AgentManager:
    ...
    def _create_chat_client(self, agent_config: dict):
        ...
        # The Responses API lives on the '/openai/v1' surface, which only
        # accepts the non-dated api-version values ('preview', 'v1', 'latest').
        # Dated versions like '2024-02-15-preview' (the default, and the kind
        # shown in the Foundry portal URL for chat/completions) are rejected
        # there with HTTP 400 "api version not supported". Operators routinely
        # paste a dated version into the AOAI endpoint config, so coerce it to
        # 'preview' for Responses clients unless they already chose a valid v1
        # value.
        if use_responses and (api_version or "").lower() not in ("preview", "v1", "latest"):
            logger.info(f"[AOAI-CONFIG] Agent '{agent_name}': coercing api_version "
                        f"'{api_version}' -> 'preview' for Responses API endpoint "
                        f"(type={endpoint_type}); dated versions are not supported on /openai/v1")
            api_version = "preview"
