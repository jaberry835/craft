...
from .analyzers import ComputerVisionAnalyzer, DocumentIntelligenceAnalyzer, FaceAnalyzer, FaceEmbedder
...
class ImageIndexer:
    ...
    def __init__(self, settings: Settings | None = None):
        ...
        # Initialize local face embedder (deepface)
        self.face_embedder: FaceEmbedder | None = None
        if self.settings.enable_face_embeddings:
            self.face_embedder = FaceEmbedder(self.settings)
            self.logger.info("Local face embedder enabled", model=self.settings.deepface_model_name)
    ...
    async def process_image(self, file_path: str, file_url: str | None = None) -> tuple[ImageDocument, list[FaceDocument]]:
        """
        ...
        Returns:
            Tuple of (ImageDocument, list of FaceDocuments)
        """
        ...
        self.logger.info(
            ...
            face_embeddings=len(face_documents),
        )
        
        return document, face_documents
    ...
    async def index_directory(
        ...
        # Process in batches
        results = {"processed": 0, "uploaded": 0, "failed": 0, "errors": [], "face_docs_uploaded": 0}
        batch = []
        face_batch: list[FaceDocument] = []
        ...
        for image_path in image_files:
            try:
                ...
                document, face_docs = await self.process_image(str(image_path), file_url)
                batch.append(document)
                face_batch.extend(face_docs)
                results["processed"] += 1
                ...
                if len(batch) >= self.settings.batch_size:
                    ...
                    if face_batch:
                        face_result = await self.search_manager.upload_face_documents(face_batch)
                        results["face_docs_uploaded"] += face_result["uploaded"]
                        face_batch = []
        ...
        async with BlobStorageClient(self.settings) as blob_client:
            ...
            for blob_info in blobs:
                try:
                    ...
                    # Process the image
                    document, face_docs = await self.process_image_data(
                        image_data=image_data,
    ...
    async def process_image_data(
        ...
        blob_url: str,
        container_name: str
    ) -> tuple[ImageDocument, list[FaceDocument]]:
        """
