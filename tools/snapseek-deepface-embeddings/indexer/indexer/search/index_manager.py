...
from ..models import ImageDocument, FaceDocument
...
class SearchIndexManager:
    ...
    def __init__(self, settings: Settings):
        ...
        # Faces index client (for local face embeddings)
        self.faces_search_client = SearchClient(
            endpoint=settings.azure_search_endpoint,
    ...
    def _create_faces_index_definition(self) -> SearchIndex:
        """Create the faces search index definition for face embeddings."""
        ...
        vector_search = VectorSearch(
            ...
            profiles=[
                VectorSearchProfile(
                    name="vector-profile-face",
                    algorithm_configuration_name="hnsw-faces",
                )
            ],
        )
        ...
        fields = [
            ...
            SimpleField(name="face_index", type=SearchFieldDataType.Int32, filterable=True),
            ...
            SearchField(name="person_name", type=SearchFieldDataType.String, searchable=True, filterable=True),
        ...
        return SearchIndex(
            name=self.settings.azure_search_faces_index_name,
            fields=fields,
            vector_search=vector_search,
        )
    ...
    async def create_or_update_index(self) -> None:
        """Create or update the search index (images + faces)."""
        ...
        # Also create/update the faces index if face embeddings are enabled
        if self.settings.enable_face_embeddings:
            try:
                faces_index = self._create_faces_index_definition()
                self.index_client.create_or_update_index(faces_index)
                self.logger.info("Faces search index created/updated successfully",
                               index_name=self.settings.azure_search_faces_index_name)
            except Exception as e:
                self.logger.error("Failed to create/update faces index", error=str(e))
                raise
    
    async def delete_index(self) -> bool:
        """
        Delete the search index (and faces index if face embeddings are enabled).
        ...
        try:
            ...
            # Also delete faces index if face embeddings are enabled
            if self.settings.enable_face_embeddings:
                try:
                    self.index_client.delete_index(self.settings.azure_search_faces_index_name)
                    self.logger.info("Faces index deleted successfully",
                                   index_name=self.settings.azure_search_faces_index_name)
                except Exception:
                    self.logger.info("Faces index does not exist or could not be deleted")
    ...
    async def upload_face_documents(self, face_docs: list[FaceDocument]) -> dict:
        """
        Upload face documents to the faces search index.

        Args:
            face_docs: List of FaceDocument objects with embeddings.

        Returns:
            Upload result summary {uploaded, failed}.
        """
        if not face_docs:
            return {"uploaded": 0, "failed": 0}

        self.logger.info("Uploading face documents", count=len(face_docs))

        try:
            search_docs = [doc.to_search_document() for doc in face_docs]
            result = self.faces_search_client.upload_documents(documents=search_docs)

            succeeded = sum(1 for r in result if r.succeeded)
            failed = len(result) - succeeded

            self.logger.info("Face document upload complete", succeeded=succeeded, failed=failed)
            return {"uploaded": succeeded, "failed": failed}

        except Exception as e:
            self.logger.error("Face document upload failed", error=str(e))
            raise

    async def get_face_document_count(self) -> int:
        """Get the total number of face documents in the faces index."""
        try:
            return self.faces_search_client.get_document_count()
        except Exception:
            return 0
