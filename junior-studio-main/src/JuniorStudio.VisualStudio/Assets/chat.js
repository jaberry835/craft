...
(function() {
    ...
    const ctActiveTitle = document.getElementById('ct-active-title');
    if (btnNewChat) {
        btnNewChat.addEventListener('click', function () {
            if (historyPanel) historyPanel.classList.remove('open');
            if (btnToggleHistory) btnToggleHistory.classList.remove('active');
    ...
    if (btnToggleHistory) {
        btnToggleHistory.addEventListener('click', function () {
            toggleHistoryPanel();
            btnToggleHistory.classList.toggle('active', historyPanel.classList.contains('open'));
        });
    }
    document.addEventListener('click', function (e) {
        if (!historyPanel || !historyPanel.classList.contains('open')) return;
        if (historyPanel.contains(e.target)) return;
        if (btnToggleHistory && btnToggleHistory.contains(e.target)) return;
    ...
    // ── Token usage (per-session, accumulated client-side) ──
    // Context-window sizes per model family. Picked by substring match against the
    // active deployment id (case-insensitive). Falls back to 128K.
    const CONTEXT_WINDOWS = [
        { match: /gpt-?5/i,                  size: 400000, label: 'gpt-5' },
        { match: /gpt-?4\.1/i,               size: 1000000, label: 'gpt-4.1' },
        { match: /gpt-?4o|chatgpt-?4o/i,     size: 128000,  label: 'gpt-4o' },
        { match: /\bo[1-4](-|$)/i,           size: 200000,  label: 'o-series' },
        { match: /gpt-?4(-|$)|gpt4(-|$)/i,   size: 128000,  label: 'gpt-4' },
        { match: /gpt-?3\.?5/i,              size: 16385,   label: 'gpt-3.5' }
    ];
    let tokenContextWindow = 128000;
    let tokenContextWindowLabel = 'context';
    function setTokenContextWindowFromDeployment(deploymentId) {
        const id = String(deploymentId || '');
        for (const entry of CONTEXT_WINDOWS) {
            if (entry.match.test(id)) {
                tokenContextWindow = entry.size;
                tokenContextWindowLabel = entry.label;
                renderTokenMeter();
                return;
            }
        }
        tokenContextWindow = 128000;
        tokenContextWindowLabel = 'context';
        renderTokenMeter();
    }
    const sessionTokenStats = {
        requests: 0,
        input: 0, output: 0, total: 0, reasoning: 0, cachedInput: 0,
        lastInput: 0, lastOutput: 0, lastTotal: 0, lastReasoning: 0, lastCachedInput: 0
    };
    function resetTokenStats() {
        sessionTokenStats.requests = 0;
        sessionTokenStats.input = sessionTokenStats.output = sessionTokenStats.total = 0;
        sessionTokenStats.reasoning = sessionTokenStats.cachedInput = 0;
        sessionTokenStats.lastInput = sessionTokenStats.lastOutput = sessionTokenStats.lastTotal = 0;
        sessionTokenStats.lastReasoning = sessionTokenStats.lastCachedInput = 0;
        renderTokenMeter();
    }
    ...
    function renderTokenMeter() {
        ...
        if (!meterEl) return;
        const label = meterEl.querySelector('.meter-label');
        const total = sessionTokenStats.total;
        const pct = Math.min(100, Math.round((total / tokenContextWindow) * 100));
        if (label) {
            label.textContent = formatTokens(total) + ' / ' + formatTokens(tokenContextWindow) + ' (' + pct + '%)';
        }
        const lines = [
            'Session token usage (' + tokenContextWindowLabel + ')',
            '─────────────────────────',
            'Total:        ' + total.toLocaleString(),
            '  Input:      ' + sessionTokenStats.input.toLocaleString()
                + (sessionTokenStats.cachedInput > 0
                    ? '  (cached ' + sessionTokenStats.cachedInput.toLocaleString() + ')'
                    : ''),
            '  Output:     ' + sessionTokenStats.output.toLocaleString()
                + (sessionTokenStats.reasoning > 0
                    ? '  (reasoning ' + sessionTokenStats.reasoning.toLocaleString() + ')'
                    : ''),
            'Requests:     ' + sessionTokenStats.requests,
            '',
            'Last turn:    ' + sessionTokenStats.lastTotal.toLocaleString(),
            '  Input:      ' + sessionTokenStats.lastInput.toLocaleString()
                + (sessionTokenStats.lastCachedInput > 0
                    ? '  (cached ' + sessionTokenStats.lastCachedInput.toLocaleString() + ')'
                    : ''),
            '  Output:     ' + sessionTokenStats.lastOutput.toLocaleString()
                + (sessionTokenStats.lastReasoning > 0
                    ? '  (reasoning ' + sessionTokenStats.lastReasoning.toLocaleString() + ')'
                    : ''),
            '',
            pct + '% of ' + tokenContextWindow.toLocaleString() + ' context window'
        ];
        meterEl.title = lines.join('\n');
        if (pct > 75)      meterEl.style.color = 'var(--vscode-editorError-foreground, #f44)';
        else if (pct > 50) meterEl.style.color = 'var(--vscode-editorWarning-foreground, #fa0)';
        else               meterEl.style.color = '';
    }
    renderTokenMeter();
    ...
    function sendCurrentMessage() {
        ...
        // Flip the send button to the spinning stop indicator immediately so the
        // user has visual confirmation their prompt was submitted, even before
        // the sidecar streams its first event back.
        setAgentRunning(true);
        showGlobalWorkingIndicator('Thinking');
    }
    ...
    function setAgentRunning(running) {
        ...
        if (running) {
            ...
            btnSend.innerHTML = '<span class="send-spinner"></span><span class="send-stop"></span>';
    ...
    function renderWelcomeScreen(msg) {
        removeWelcomeScreen();
        ...
        if (prompts) {
            ...
            prompts.forEach(function (p) {
                ...
                var iconName = p.icon || 'edit';
                btn.innerHTML = '<i class="codicon codicon-' + escapeHtml(iconName) + '"></i><span class="welcome-prompt-text">' + escapeHtml(p.text || '') + '</span>';
                btn.addEventListener('click', function () {
                    var promptText = p.prompt || p.text || '';
                    if (inputEl) {
        ...
        if (workingIndicator && workingIndicator.parentNode === messagesEl) {
            messagesEl.insertBefore(screen, workingIndicator);
        } else {
    ...
    function setModels(models, activeDeployment, reasoning) {
        ...
        setTokenContextWindowFromDeployment(currentActiveDeployment);
        updateModelTrigger();
        renderModelOptions();
    ...
    function renderModelOptions() {
        ...
        filtered.forEach(function(m) {
            ...
            btn.addEventListener('click', function() {
                ...
                setTokenContextWindowFromDeployment(m.deploymentId);
    ...
    function renderApprovalPrompt(id, category, description) {
        if (!id) { return; }
        var existing = document.getElementById('approval-' + id);
        if (existing) { return; }
        var wrap = document.createElement('div');
        wrap.id = 'approval-' + id;
        wrap.className = 'approval-block';
        var labelMap = {
            write: 'File write requested',
            delete: 'File delete requested',
            shell: 'Shell command requested'
        };
        var label = labelMap[category] || ('Action requested (' + (category || 'tool') + ')');
        var safeDesc = (description || '').toString();
        var header = document.createElement('div');
        ...
        allowSessionBtn.className = 'approval-btn approval-allow-session';
        allowSessionBtn.textContent = 'Allow for session';
        allowSessionBtn.title = 'Allow this and skip future ' + (category || 'tool') + ' approvals until Visual Studio is restarted';
        var denyBtn = document.createElement('button');
        denyBtn.className = 'approval-btn approval-deny';
        denyBtn.textContent = 'Deny';
        function send(allow, scope) {
            allowBtn.disabled = true;
            allowSessionBtn.disabled = true;
            denyBtn.disabled = true;
            wrap.classList.add(allow ? 'resolved-allow' : 'resolved-deny');
            var status = document.createElement('span');
            status.className = 'approval-status';
            if (!allow) status.textContent = 'Denied';
            else if (scope === 'session') status.textContent = 'Allowed (rest of session)';
            else status.textContent = 'Allowed';
            actions.appendChild(status);
            var payload = { type: 'approvalResponse', id: id, allow: allow };
            if (scope) payload.scope = scope;
            if (category) payload.category = category;
            vscode.postMessage(payload);
        }
        allowBtn.addEventListener('click', function () { send(true, 'once'); });
        allowSessionBtn.addEventListener('click', function () { send(true, 'session'); });
        denyBtn.addEventListener('click', function () { send(false, 'once'); });
    ...
    function copyTextToClipboard(text) {
        // navigator.clipboard is blocked in some WebView2 contexts; fall back to execCommand.
        try {
    ...
    window.addEventListener('message', (event) => {
        ...
        switch (msg.type) {
            ...
            case 'modelChanged': {
                if (modelSelectEl && msg.model) {
                    ...
                    if (opt) {
                        ...
                        setTokenContextWindowFromDeployment(opt.value);
                        updateModelTrigger();
                        renderModelOptions();
                    }
                }
            ...
            case 'approvalRequest': {
                renderApprovalPrompt(msg.id, msg.category, msg.description);
                break;
            }
            case 'imageAttached': {
                // Extension picked an image via the OS file dialog
                if (msg.dataUri) {
                    pendingImages.push(msg.dataUri);
                    refreshAttachPreview();
                }
                break;
            }
            ...
            case 'activeSession': {
                if (ctActiveTitle) ctActiveTitle.textContent = msg.title || '';
                break;
            }
            ...
            case 'tokenUsage': {
                // Sidecar emits per-turn deltas: { input, output, total, reasoning, cachedInput }.
                // We accumulate session totals client-side and update the footer meter.
                var inT  = Number(msg.input)  || 0;
                var outT = Number(msg.output) || 0;
                var totT = Number(msg.total)  || (inT + outT);
                var rT   = Number(msg.reasoning)   || 0;
                var cT   = Number(msg.cachedInput) || 0;
                sessionTokenStats.requests   += 1;
                sessionTokenStats.input      += inT;
                sessionTokenStats.output     += outT;
                sessionTokenStats.total      += totT;
                sessionTokenStats.reasoning  += rT;
                sessionTokenStats.cachedInput+= cT;
                sessionTokenStats.lastInput       = inT;
                sessionTokenStats.lastOutput      = outT;
                sessionTokenStats.lastTotal       = totT;
                sessionTokenStats.lastReasoning   = rT;
                sessionTokenStats.lastCachedInput = cT;
                renderTokenMeter();
                break;
            }
            case 'slashCommands': {
