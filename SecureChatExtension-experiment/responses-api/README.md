...
### Wire API (chat-completions vs. responses)

| Setting | Default | Meaning |
|---|---|---|
| `junior.azureOpenAI.wireApi` | `"chat-completions"` | `chat-completions` (classic `/openai/deployments/{id}/chat/completions`) or `responses` (new `/openai/v1/responses` with typed reasoning events). |
| `junior.azureOpenAI.reasoningEffort` | `"high"` | `minimal`, `low`, `medium`, or `high`. Only honored when `wireApi=responses` and the deployment is reasoning-capable. Lower = faster + cheaper. |
...
See [docs/APIM-JUNIOR-LOCAL-RESPONSES-SETUP.md](docs/APIM-JUNIOR-LOCAL-RESPONSES-SETUP.md) for an end-to-end APIM walkthrough.
...
- [docs/APIM-JUNIOR-LOCAL-RESPONSES-SETUP.md](docs/APIM-JUNIOR-LOCAL-RESPONSES-SETUP.md) — opt into the new `/openai/v1/responses` wire API for typed reasoning events and server-side state.
