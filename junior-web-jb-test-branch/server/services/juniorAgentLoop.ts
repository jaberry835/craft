...
export class JuniorAgentLoop {
  ...
  private async tryGenerateAssistantReply(context: LoopContext): Promise<string | undefined> {
    ...
    try {
      ...
      for await (const chunk of this.chatRuntime.completeWithToolsStream({
        ...
        options: {
          reasoningEffort: JuniorAgentLoop.reasoningEffortForAgent(context.activeAgent)
        },
        state: context.state
      })) {
        if (chunk.type === 'text') {
  ...
  static reasoningEffortForAgent(agent: AgentDefinition) {
    return agent.reasoningEffort ?? 'medium';
  }
