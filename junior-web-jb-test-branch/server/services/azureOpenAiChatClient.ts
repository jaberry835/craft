...
import type { AgentModelConnection, ReasoningEffort } from '../types.js';
...
export class AzureOpenAiChatClient {
  ...
  private buildRequest(
    ...
    const isV1Endpoint = /\/v1(?:\/|$)/i.test(endpoint) || isResponsesEndpoint;
    ...
    const body: Record<string, unknown> = {
      ...
      ...(!useReasoning ? { temperature: connection.temperature ?? 0.2 } : {}),
    ...
    if (isResponsesEndpoint) {
      ...
      return {
        ...
        body: {
          ...
          ...(reasoning ? { reasoning } : {}),
          ...(!useReasoning ? { temperature: connection.temperature ?? 0.2 } : {}),
          max_output_tokens: connection.maxTokens ?? 1200,
          stream
        }
      };
    }
