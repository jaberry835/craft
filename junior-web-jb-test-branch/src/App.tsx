...
import type { AdminConnectivityReport, AgentConnectionSaveRequest, AgentConnectionType, AgentDefinition, AgentGroundingSource, AgentModelConnectionStatus, AgentResponse, AgentTemplateDefinition, AzureAiSearchGroundingSource, AzureAuthMode, AzureCloud, ChatMessage, ChatMessageDisplayPart, ChatSessionSummary, ConnectivityCheck, ConnectivitySection, McpCatalogEntry, McpCustomHeader, McpServerAuthMode, McpServerStatus, ReasoningEffort, WorkspaceFile, WorkspaceIndex, WorkspaceSummary, WorkspaceTemplateDefinition, WorkspaceTreeNode } from './types/workbench';
import './App.css';
...
function App() {
  ...
  const [configView, setConfigView] = useState<'workspace' | 'connectors' | 'mcp' | 'agents' | 'workspace-templates' | 'connectivity'>('connectors');
  ...
  useEffect(() => {
    void Promise.all([
      workbenchApi.getWorkspaces().then(setWorkspaces),
      refreshChatSessions(),
      refreshWorkspace(),
  ...
  }, [activeScreen, configView, refreshAdminConnectivity]);
  ...
  useEffect(() => {
    ...
    setCustomAgentReasoningEffortDraft(selectedConfigAgent.reasoningEffort ?? defaultReasoningEffort);
  ...
  function openConfigDrawer(view: 'connectors' | 'mcp' | 'agents' | 'workspace-templates' | 'connectivity') {
    setConfigView(view);
    ...
    <main className={`workbench-shell theme-${themeMode}`}>
      <header className="topbar">
        ...
        <div className="topbar-actions">
          ...
          <button type="button" className="topbar-settings-button" onClick={() => openConfigDrawer(configView === 'workspace' ? 'workspace-templates' : configView)} title="Open admin">
            <Settings size={17} />
            Admin
          </button>
        </div>
                  ...
                  <div className="config-item-list">
                    ...
                    {workspaceSettingsConnections.map((connection) => (
                      <button key={connection.id} type="button" className={selectedConnectorId === connection.id ? 'config-item-row selected' : 'config-item-row'} onClick={() => selectConnectorForEditing(connection.id)}>
                        ...
                        <span>{connection.type === 'azure-openai' ? 'Azure OpenAI' : 'Azure AI Search'}</span>
                      </button>
                    ))}
                  </div>
                </>
