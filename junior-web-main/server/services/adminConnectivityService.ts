...
export class AdminConnectivityService {
  ...
  private getAiSection(): ConnectivitySection {
    const checks = this.agentConfigStore.listConnections()
      ...
      .map((connection) => this.toAiCheck(connection));
  ...
  private toAiCheck(connection: AgentConnectionStatus): ConnectivityCheck {
    const endpoint = connection.endpoint?.trim();
    const endpointHost = this.describeEndpointHost(endpoint);
    const details: string[] = [connection.type === 'azure-openai' ? 'Azure OpenAI' : 'Azure AI Search'];

    if (endpointHost) {
      details.push(`Endpoint ${endpointHost}`);
    }

    details.push(`Auth ${connection.authMode === 'entra' ? 'Entra ID' : 'API key'}`);

    if (connection.type === 'azure-openai') {
      const scope = connection.credentialScope ?? this.defaultAzureOpenAiScope(connection.cloud, endpoint, connection.endpointKind ?? 'auto');
      details.push(`Endpoint type ${this.azureOpenAiEndpointKindLabel(endpoint, connection.endpointKind ?? 'auto')}`);
      details.push(`Scope ${scope}`);

      if (connection.deployment) {
        details.push(`Deployment ${connection.deployment}`);
      }
    }

    if (connection.type === 'azure-ai-search' && connection.indexNames?.length) {
      details.push(`Indexes ${connection.indexNames.join(', ')}`);
    }
    ...
    return {
      ...
      message: this.aiConnectionMessage(connection, endpoint),
      detail: details.join(' | ')
    };
  }

  private aiConnectionMessage(connection: AgentConnectionStatus, endpoint?: string): string {
    if (!connection.configured) {
      return `Missing ${connection.missing.join(', ') || 'required settings'}`;
    }

    if (connection.type === 'azure-openai' && connection.authMode === 'entra' && /\/api\/projects\//i.test(endpoint ?? '')) {
      return 'Configured. Foundry project endpoints still require Foundry RBAC on the project scope.';
    }

    return 'Configured';
  }

  private describeEndpointHost(endpoint?: string): string | undefined {
    if (!endpoint) {
      return undefined;
    }
  ...
  private azureOpenAiEndpointKindLabel(endpoint: string | undefined, endpointKind: AzureOpenAiEndpointKind): string {
    const resolved = this.resolveAzureOpenAiEndpointKind(endpoint, endpointKind);
    if (resolved === 'foundry-project') {
      return 'Foundry project';
    }

    if (resolved === 'openai-v1') {
      return 'OpenAI-compatible v1';
    }

    if (resolved === 'azure-openai-legacy') {
  ...
  private resolveAzureOpenAiEndpointKind(endpoint: string | undefined, endpointKind: AzureOpenAiEndpointKind): Exclude<AzureOpenAiEndpointKind, 'auto'> | 'unresolved' {
  ...
  private defaultAzureOpenAiScope(cloud: AgentConnectionStatus['cloud'], endpoint: string | undefined, endpointKind: AzureOpenAiEndpointKind): string {
    const resolved = this.resolveAzureOpenAiEndpointKind(endpoint, endpointKind);
