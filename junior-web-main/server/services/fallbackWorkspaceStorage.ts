...
export class FallbackWorkspaceStorage implements WorkspaceStorage {
  ...
  async readTextFile(relativePath: string): Promise<WorkspaceFile> {
    return this.run(
      () => this.primary.readTextFile(relativePath),
      () => this.fallback.readTextFile(relativePath),
      { degradeOnError: (error) => !isMissingPathError(error) }
    );
  }
  ...
  private async run<T>(
    primary: () => Promise<T>,
    fallback: () => Promise<T>,
    options: { degradeOnError?: (error: unknown) => boolean } = {}
  ): Promise<T> {
    if (this.degraded) {
      return fallback();
    }
    ...
    try {
      ...
      if (options.degradeOnError && !options.degradeOnError(error)) {
        throw error;
      }
...
function isMissingPathError(error: unknown): boolean {
  ...
  if (candidate.code === 'ENOENT' || candidate.code === 'BlobNotFound' || candidate.code === 'ContainerNotFound') {
    return true;
  }

  return candidate.details?.errorCode === 'BlobNotFound' || candidate.details?.errorCode === 'ContainerNotFound';
}
