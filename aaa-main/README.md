...
Built-in tools are `list_files`, `read_file`, `search_files`, `write_file`, `edit_file`, `copy_path`, `browser_capture`, and `load_skill`. Agent `tools` tokens map as follows: `read` → list/read, `search` → list/search, `edit` → write/edit/copy, `browser` or `screenshot` → Edge evidence capture, and `<server>/*` or `<server>/<tool>` → MCP tools. `execute` is ignored: AAA never runs commands or scripts, so skills should copy bundled template files with `copy_path` rather than calling a script. `write_file` creates missing parent folders, and `copy_path` never overwrites existing files. Disabling an agent, skill, MCP server, or built-in tool in **Project customizations** removes it from the next run.
...
Navigation accepts user-entered absolute HTTP and HTTPS addresses and rejects other schemes. Captures are PNG files under `evidence\screenshots\` by default. Every PNG has an adjacent JSON provenance record containing the source URL, capture time, Edge mode, full-page setting, and screenshot path. The `capture-web-evidence` skill guides the model, while the deterministic `browser_capture` tool owns launch, navigation, capture, and close behavior.
