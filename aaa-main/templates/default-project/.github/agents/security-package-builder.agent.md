...
tools: [read, search, edit, browser, mcp-publisher/*]
agents: []
user-invocable: true
---
...
1. Use the `initialize-security-package` skill if the package structure or configuration is absent. Copy its bundled template instead of manually recreating the directory tree.
...
5. Use the `capture-web-evidence` skill when authenticated portal screenshots are requested.
6. Use the `create-architecture-diagram` skill for grounded SVG architecture, CONOPS, or workflow diagrams.
7. Use the `validate-security-package` skill after generating or changing responses.
8. Summarize determinations, gaps, and files changed. If requested, publish reviewed Markdown through `mcp-publisher`.
