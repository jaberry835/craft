...
The other skill we should for this is something that will create svg archiecture diagrams , conops, and other documents based on the information that we have from background and other information.  We must make sure we can display these svgs (and also images) 

We will later ask the best way to build a dymanic architecture diagram based on our cloudscan output that we can host within the application (there is a great react component for this) ... hold off on this for now but add to relevant punch lists. 

We must fix dark mode, as many of the text preview boxes don't show correctly and the text is way too dark .  We also must fix if identity json previewing that correctly 
