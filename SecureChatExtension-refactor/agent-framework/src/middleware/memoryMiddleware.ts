...
export class MemoryMiddleware implements AgentMiddleware {
    ...
    async process(context: AgentContext, next: () => Promise<AgentResponse>): Promise<AgentResponse> {
        // Inject memory before the agent runs
        this.injectMemory(context);

        const response = await next();

        // Record results after the agent finishes
        this.recordResults(context, response);

        return response;
    }

    private injectMemory(context: AgentContext): void {
    ...
    private recordResults(_context: AgentContext, _response: AgentResponse): void {
        // Tool result recording is handled per-tool-call in the agent loop.
        // This is a hook point for post-run aggregate recording if needed.
    }
