 ...
 *
 * Future iterations may refactor this into a ResponseStream transform hook.
 */
...
export class StreamBufferMiddleware {
    ...
    onTextChunk(text: string): void {
        ...
        if (this.toolCallDetected) {
            // Tool calls already detected — text is narration, don't buffer
            return;
        }
    ...
    finalize(): { narrationText: string | null; assistantText: string; bubbleOpen: boolean; textAlreadyRendered: boolean } {
        ...
        const narration = (this.toolCallDetected && this.pendingTextBuffer && !this.textAlreadyRendered)
