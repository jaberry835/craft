 ...
 *  - ProgressMiddleware:      working block UI tracking
...
import { MiddlewarePipeline, type AgentContext } from './framework/middleware';
...
/** Max auto-fix cycles when the model stops but errors remain */
const MAX_AUTOFIX_CYCLES = 3;

/** Max emergency recovery attempts for API rejections */
const MAX_CONTEXT_RECOVERY_ATTEMPTS = 3;
// TODO RUDE: remove if causing problems — revert to no stall retry constant
...
export class AgentLoop {
    ...
    private chatClient: AoaiChatClientAdapter;
    ...
    constructor(
        ...
        this.chatClient = new AoaiChatClientAdapter(aoaiClient);
    ...
    async run(userMessage: string, images?: string[], files?: { name: string; content: string }[], displayText?: string): Promise<void> {
        ...
        let streamStallRetries = 0; // TODO RUDE: remove if causing problems
        ...
        // ── Step 5: Core iteration loop ──
        let finalizeWorkingUi: (() => void) | undefined;
        try {
            ...
            while (iteration < this.maxIterations && this.running) {
                ...
                // ── Stream from LLM using the ChatClient adapter ──
                ...
                try {
                    const stream = this.aoaiClient.streamChat(
                        requestMessages, tools, this.abortController!.signal, { reasoningMode }
                    );
                    ...
                    // ── Context recovery (via RecoveryMiddleware logic) ──
                    const isInvalidPrompt = streamErr.statusCode === 400 &&
                        (streamErr.errorCode === 'invalid_prompt' || /invalid.prompt/i.test(streamErr.message));

                    // TODO RUDE: remove this isParamRejection block if causing problems.
                    // Revert to just the isInvalidPrompt check below (remove the || isParamRejection branch).
                    // --- begin broadened 400 recovery ---
                    const isParamRejection = streamErr.statusCode === 400 && !isInvalidPrompt &&
                        /unknown.parameter|unsupported.parameter|unrecognized.request|stream_options|max_completion_tokens|tool_choice/i.test(streamErr.message);
                    // --- end broadened 400 recovery ---

                    if ((isInvalidPrompt || isParamRejection) && contextRecoveryAttempts < MAX_CONTEXT_RECOVERY_ATTEMPTS) {
                        ...
                        if (contextRecoveryAttempts === 1) {
                            this.callbacks.sendToWebview({ type: 'setStatus', status: 'Prompt too large for model — trimming context...' });
                            this.messages = this.contextManager.emergencyTrim(this.messages);
                        } else if (contextRecoveryAttempts === 2) {
                            reasoningMode = true;
                            this.callbacks.sendToWebview({ type: 'setStatus', status: 'Retrying with reasoning-compatible parameters...' });
                        } else {
                            ...
                            if (fallback) {
                                ...
                                reasoningMode = false;
                                this.messages = this.contextManager.emergencyTrim(this.messages);
                                this.callbacks.sendToWebview({ type: 'setStatus', status: `Switching to ${fallback.name || fallback.deploymentId} to continue...` });
                            } else {
                                throw streamErr;
                            }
                        }
                    ...
                    // TODO RUDE: remove this entire stream-stall retry block if causing problems.
                    // Revert to just the throw below (remove this if block entirely).
                    // --- begin stream-stall retry ---
                    const isStreamStall = /stream stalled|no data received/i.test(streamErr.message);
                    if (isStreamStall && streamStallRetries < MAX_STREAM_STALL_RETRIES) {
                        streamStallRetries++;
                        this.log?.(`[WARN] Stream stall retry ${streamStallRetries}/${MAX_STREAM_STALL_RETRIES}: ${streamErr.message}`);
                        this.callbacks.sendToWebview({ type: 'setStatus', status: `Server stalled — retrying (${streamStallRetries}/${MAX_STREAM_STALL_RETRIES})...` });
                        continue;
                    }
                    // --- end stream-stall retry ---
                ...
                streamStallRetries = 0; // TODO RUDE: remove if causing problems
                if (!this.running) { break; }
                ...
                // ── No tool calls → check autofix or finish ──
                if (toolCalls.length === 0) {
            ...
            // Ensure any in-progress working block is closed so it doesn't stay stuck as "Working..."
            finalizeWorkingUi?.();
        } finally {
    ...
    private async checkEditedFileDiagnostics(): Promise<string | null> {
        ...
        if (errors.length > 20) { capped.push(`... and ${errors.length - 20} more errors.`); }
        return capped.join('\n');
    }

    private async executeTool(name: string, args: Record<string, unknown>): Promise<ToolResult> {
        if (this.cancelled) { return { success: false, result: 'Cancelled by user.' }; }
        ...
        if (name.startsWith('mcp_')) { return this.mcpClient.callTool(name, args); }
    ...
    private async executeToolWithRetry(name: string, args: Record<string, unknown>): Promise<ToolResult> {
        ...
        if (result.result.includes('User declined') || result.result.includes('Invalid path')) { return result; }
        await new Promise(r => setTimeout(r, 600));
        const retry = await this.executeTool(name, args);
        if (!retry.success) { retry.result = `[Retry also failed] ${retry.result}`; }
        return retry;
    }
