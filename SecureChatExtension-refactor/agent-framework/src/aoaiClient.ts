...
export class AzureOpenAIClient {
    ...
    async *streamChat(
        ...
        // (i.e. remove isNativeProvider guard and always send stream_options when API version supports it)
        // stream_options is only reliably supported by OpenAI and Azure OpenAI;
        // many OAI-compatible endpoints (vLLM, LiteLLM, etc.) reject it with 400.
        const isNativeProvider = config.provider === 'openai' || config.provider === 'direct';
    ...
    private async httpRequestWithRetry(
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                return await this.httpRequest(url, currentBody, apiKey, abortSignal, provider);
            } catch (err: any) {
                // TODO RUDE: remove if causing problems — revert to:
                //   const retryable = err.statusCode === 429 || err.statusCode === 503;
                // (i.e. remove 500 and 502 from retryable set)
                const retryable = err.statusCode === 429 || err.statusCode === 500
                    || err.statusCode === 502 || err.statusCode === 503;

                // TODO RUDE: remove this entire param-stripping block if causing problems.
                // Revert to just: if (!retryable || attempt === maxRetries) { throw err; }
                // --- begin param-stripping 400 recovery ---
                ...
                // --- end param-stripping 400 recovery ---
