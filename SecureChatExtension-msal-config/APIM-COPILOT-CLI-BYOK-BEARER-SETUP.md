...
## Caller App Registration (MSAL clients)

If your callers will acquire tokens via MSAL (the Junior extension's `msal` mode, an `az` CLI script, a Postman collection, etc.) you need a public-client app registration in the same tenant APIM validates against. One-time setup:

1. **App registrations → New registration** → name it (e.g. `apim-bearer-cli`), pick the supported account types you want, **no redirect URI yet** → Register.
2. **Authentication → + Add a platform → Mobile and desktop applications** → tick the suggested `http://localhost` redirect URI → Configure. *Required for every MSAL client* — without it interactive sign-in fails with `AADSTS500113: No reply address is registered for the application.`
3. **Authentication → Advanced settings → Allow public client flows = Yes**.
4. **Expose an API**:
    - Set **Application ID URI** (the default `api://<clientId>` is fine; this is what the validate-jwt audience must match).
    - **Add a scope** named `user_impersonation`, "Admins and users" can consent, state Enabled.
    - (Optional) **Authorized client applications → Add** the well-known Azure CLI client id `04b07795-8ddb-461a-bbee-02f9e1bf7b46` with the `user_impersonation` scope checked. This lets you test with `az account get-access-token --scope api://<your-app>/.default` from any signed-in shell without a separate consent prompt.
5. **API permissions**: nothing extra is required for the APIM-bearer scenario. The token audience is **your own** app reg's `user_impersonation` scope, and a client never needs an API permission entry to request its own scopes. (Azure Cognitive Services / Microsoft Graph permissions only become relevant if you also acquire tokens for *other* resources.)

After save, the audience your `validate-jwt` policy must accept is `api://<clientId>` (and/or the bare `<clientId>` GUID — both forms appear depending on token version):
...
Smoke test the app reg from a normal shell before wiring it into Junior:

```powershell
az account get-access-token --scope "api://<clientId>/.default" --query accessToken -o tsv
```

That returning a JWT with `aud` set to `api://<clientId>` (decode at jwt.ms) confirms the app reg is configured correctly.
