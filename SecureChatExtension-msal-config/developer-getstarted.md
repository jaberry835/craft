...
For **custom sovereign tenants** ( isolated tenants), use MSAL instead — set `junior.copilotCli.providerBearerTokenSource` to `"msal"` and configure the shared `junior.msal.*` block (see the MSAL section under local Azure/APIM bearer mode below).
...
##### MSAL (bring your own app registration) — local Azure/APIM bearer mode

`vscode-auth-session` reuses VS Code's first-party Microsoft app registration. That works in commercial AAD and the published sovereign environments, but not in **custom sovereign tenants** (isolated/dev tenants where VS Code's reply URLs are not registered) — those return `AADSTS500113: No reply address is registered`.

For those environments switch Junior to MSAL:
...
```jsonc
{
  ...
  // Shared MSAL config — used by both AOAI and Copilot CLI namespaces
  "junior.msal.clientId": "<your-public-client-app-clientid>",
  "junior.msal.tenantId": "<your-tenant-guid>",
  // For commercial / Gov / China leave authority unset and use cloudInstance:
  "junior.msal.cloudInstance": "https://login.microsoftonline.com",
  // For isolated custom set authority + knownAuthorities explicitly:
  // "junior.msal.authority": "https://login.microsoftonline...gov/<tenant>",
  // "junior.msal.knownAuthorities": ["login.microsoftonline...gov"],
  // For headless workstations:
...
- **Authentication → Add a platform → Mobile and desktop applications**, redirect URI `http://localhost`. *Required for every MSAL setup* — without it sign-in fails with `AADSTS500113: No reply address is registered`.
- **Authentication → Advanced settings → Allow public client flows = Yes**.
- **API permissions**: only needed when the scope targets a Microsoft-owned resource.
    - **APIM-fronted** (scope `api://<your-app>/user_impersonation`): no extra API permission needed (it's your own app's scope). You DO need **Expose an API → add scope `user_impersonation`** so the audience exists.
    - **Direct Foundry / AOAI** (scope `https://cognitiveservices.azure.com/.default` or `https://ai.azure.com/.default`): add delegated permission for **Azure Cognitive Services → user_impersonation** (or **Azure AI Services**) and **Grant admin consent**. Without it sign-in fails with `AADSTS650057: Invalid resource`.
- **Data-plane RBAC** on the AOAI / Foundry resource: signed-in user must have **Cognitive Services OpenAI User** (for `*.cognitiveservices.azure.com`) or **Cognitive Services User** (for `*.services.ai.azure.com`). AAD will issue the token without it, but the actual call returns 401/403.

> The built-in `vscode-auth-session` doesn't need any of this because the OAuth client is VS Code's first-party Microsoft app — it has every redirect URI and broker permission already. With MSAL, your app is the OAuth client, so you take on a one-time configuration in exchange for working in custom sovereign tenants.

Tokens are cached in VS Code SecretStorage per `(clientId, authority)` and survive restarts. Commands:

- **Junior: MSAL Sign In** — sign in for the configured namespace.
