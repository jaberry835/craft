...
import { getCopilotCliApiKey } from './copilotCliSecrets';
import {
    getConfiguredAuthSource,
    getConfiguredScopes,
    getConfiguredVsCodeProviderId,
    getMsalConfig,
} from './tokenResolver';
...
/**
 * Returns true when Copilot CLI BYOK is configured to obtain its bearer
 * token from any session-style source (vscode-auth-session OR msal).
 * Used as a credential signal in the availability check.
 */
export function copilotCliHasSessionBearerSource(): boolean {
    return getConfiguredAuthSource('copilotCli') !== undefined;
}
...
export function getCopilotCliAvailability(env: NodeJS.ProcessEnv = buildCopilotCliProcessEnv()): CopilotCliAvailability {
    ...
    if (byokConfig.hasSignal) {
        ...
        if ((byokConfig.type === 'azure' || byokConfig.type === 'anthropic') && !byokConfig.apiKey && !byokConfig.bearerToken && !byokConfig.bearerAuthSession && !byokConfig.hasMsalSource) {
...
export function getCopilotCliBearerAuthSessionConfig(): CopilotCliBearerAuthSessionConfig | undefined {
    if (getConfiguredAuthSource('copilotCli') !== 'vscode-auth-session') {
        return undefined;
    }
    return {
        providerId: getConfiguredVsCodeProviderId('copilotCli'),
        scopes: getConfiguredScopes('copilotCli'),
    };
}
...
function getByokConfig(env: NodeJS.ProcessEnv): CopilotCliByokConfig {
    ...
    const apiKey = (getCopilotCliApiKey() || getSetting<string>('copilotCli.providerApiKey') || env.COPILOT_PROVIDER_API_KEY || '').trim();
    ...
    const hasMsalSource = getConfiguredAuthSource('copilotCli') === 'msal' && Boolean(getMsalConfig());
    const hasSignal = [
        baseUrl,
        model,
        apiKey,
