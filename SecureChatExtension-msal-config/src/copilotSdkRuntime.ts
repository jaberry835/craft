...
import { getCopilotCliApiKeyAsync } from './copilotCliSecrets';
import { resolveBearerToken } from './tokenResolver';
...
export class CopilotSdkRuntime implements AgentRuntime {
    ...
    private async buildProviderConfig(): Promise<ProviderConfig | undefined> {
        ...
        const apiKey = (await getCopilotCliApiKeyAsync()) || getSetting<string>('copilotCli.providerApiKey') || cliEnv.COPILOT_PROVIDER_API_KEY || undefined;
    ...
    private async resolveProviderBearerToken(cliEnv: NodeJS.ProcessEnv): Promise<string | undefined> {
        const resolved = await resolveBearerToken('copilotCli', { interactive: true });
        if (resolved?.accessToken) {
            const tokenClaims = this.decodeSafeJwtClaims(resolved.accessToken);
            const expiry = typeof tokenClaims?.exp === 'number'
                ? new Date(tokenClaims.exp * 1000).toISOString()
                : (resolved.expiresOn ? resolved.expiresOn.toISOString() : 'unknown');

            if (resolved.source === 'msal') {
                this.log?.(
                    `[copilot-sdk] Using MSAL for Copilot CLI bearer mode ` +
                    `(authority=${resolved.authority}, clientId=${resolved.clientId}, account=${resolved.accountLabel ?? 'unknown'})`
                );
            } else {
                ...
                this.log?.(
                    ...
                    `(provider=${authSessionConfig?.providerId ?? 'unknown'}, account=${resolved.accountLabel ?? 'unknown'}, ` +
                    `scopes=${JSON.stringify(authSessionConfig?.scopes ?? [])})`
                );
            }
            this.log?.(
        ...
        // Fall back to a manually configured bearer token from settings/env when
        // no session-based source is configured.
