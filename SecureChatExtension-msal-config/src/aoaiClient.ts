...
type AzureOpenAIAuthMode = 'api-key' | 'bearer-token' | 'vscode-auth-session' | 'msal';
...
export class AzureOpenAIClient {
    ...
    getAuthMode(provider: 'direct' | 'apim' | 'openai'): AzureOpenAIAuthMode {
        ...
        if (configured === 'bearer-token' || configured === 'vscode-auth-session' || configured === 'msal') {
            return configured;
        }
    ...
    private async resolveSessionToken(): Promise<string> {
        const resolved = await resolveBearerToken('azureOpenAI', { interactive: true });
...
export function getAzureOpenAIBearerAuthSessionConfig(): AzureOpenAIBearerAuthSessionConfig | undefined {
    return getVsCodeAuthSessionConfig('azureOpenAI');
}
